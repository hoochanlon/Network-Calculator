...
from core.base_supernet import get_supernet
...
def network_merge_calculator(parent_frame):
    # 创建输入框和标签
    label = tk.Label(parent_frame, text="请输入CIDR格式的网段，以换行做分隔进行合并：")
    label.pack(pady=5)
    ...
    def merge_networks():
        ...
        try:
            ...
            # 合并网段
            ips = [str(network.network_address) for network in validated_networks]
            merged_network = get_supernet(ips)

            # 更新显示结果
            result_text.delete("1.0", "end")  # 清空之前的结果
            result_text.insert("1.0", merged_network)  # 插入合并后的超网结果

            # 保存当前输入和结果，以便下次保留
            network_merge_state["input"] = raw_input
            network_merge_state["result"] = merged_network
            network_merge_state["error"] = ""  # 清除错误信息

            # 保存历史记录
            save_to_history("超网计算器", {"network_merge_input": raw_input}, merged_network)
